...
var month = ["జనవరి", "ఫిబ్రవరి", "మార్చి", "ఏప్రిల్", "మే", "జూన్", "జూలై", "ఆగష్టు", "సెప్టెంబర్", "అక్టోబర్", "నవంబర్", "డిసెంబర్"];

var zn = ["మేషం", "వృషభం", "మిథునం", "కర్కాటకం", "సింహం", "కన్యా", "తులా", "వృశ్చికం", "ధనుస్సు", "మకరం", "కుంభం", "మీనం"];

var wd = "ఆదివారము     సోమవారం     మంగళవారం    బుధవారం    గురువారం    శుక్రవారము    శనివారము   ";

var range = [1, 31, 0, 0, -3000, 4000, 0, 23, 0, 59, -12, 12, 0, 59];

var naks = ["Aswini", "Bharani", "Krithika/Karthigai", "Rohini", "Mrigasira/Mrigasheersham", "Aardhra/Thiruvaathirai", "Punarvasu/Punarpoosam", "Pushyami/Poosam", "Ashlesha/Aayilyam", "Magha/Makam", "Poorva Phalguni/Pooram", "Uttara Phalguni/Uthiram", "Hastha/Hastham", "Chitra/Chithirai", "Swaathi", "Vishaaka/Visaakam", "Anuraadha/Anusham", "Jyeshta/Kettai", "Moola/Moolam", "Poorvashada/Pooraadam", "Utharashada/Uthiraadam", "Shraavan/Thiruvonam", "Dhanishta/Avittam", "Shathabisha/Chathayam", "Poorvabhadhra/Poorattathi", "Uttarabhadhra/Uthirattathi", "Revathi"];
// var naks = ["అశ్విని","భరణి","కృతిక","రోహిణి","మ్రిగాసిర","ఆరుద్ర","పునర్వసు","పుష్య","ఆశ్లేష","మాఘ","పూర్వ ఫల్గుని","ఉత్తర ఫల్గుని","హస్త","చిత్ర","స్వాతి","విశాక","అనురాధ","జ్యేష్ట","మూలా","పూర్వాషాఢ","ఉత్తరాషాఢ","శ్రవణ","ధనిశ్త","శతభిష","పూర్వాభాద్ర","ఉత్తరాభాద్ర","రేవతి"];
...
var kar = ["బవ", "బాలవ", "కౌలవ", "తైతిలె", "గరజె", "వణిక్", "విష్టి", "శకుని", "చతుష్టాత్", "నాగవాన్", "కింస్తుఘ్న"];

var yog = ["విశ్కంభ", "ప్రీతి", "ఆయుష్మాన", "సౌభాగ్య", "శోభన", "అతిగండ", "సుకర్మ", "ధృతి", "శూల", "గండ", "వృద్ధి", "ధ్రువ", "వ్యఘాత", "హర్షణ", "వజ్ర", "సిద్ధి", "వ్యతీపాత", "వరియాన్", "పరిఫ", "శివ", "సిద్ధ", "సాధ్య", "శుబ", "శుక్ల", "బ్రహ్మ", "ఐంద్ర", "వైధృతి"];
...
// naks[i++] = new naksdata("అశ్విని"     ,"light"     ,"#00CCFF");

// naks[i++] = new naksdata("భరణి"    ,"awful"     ,"#7D2000");

// naks[i++] = new naksdata("కృతిక"   ,"mixed"     ,"#CC00FF");

// naks[i++] = new naksdata("రోహిణి"     ,"fixed"     ,"#0000FF");

// naks[i++] = new naksdata("మ్రిగాసిర"  ,"soft"      ,"#11AC0D");

// naks[i++] = new naksdata("ఆరుద్ర"      ,"sharp"     ,"#FF0000");

// naks[i++] = new naksdata("పునర్వసు"  ,"mobile"    ,"#FF9900");

// naks[i++] = new naksdata("పుష్య"    ,"light"     ,"#00CCFF");

// naks[i++] = new naksdata("ఆశ్లేష"     ,"sharp"     ,"#FF0000");

// naks[i++] = new naksdata("మాఘ"      ,"awful"     ,"#7D2000");

// naks[i++] = new naksdata("పూర్వ ఫల్గుని" ,"awful"     ,"#7D2000");

// naks[i++] = new naksdata("ఉత్తర ఫల్గుని" ,"fixed"     ,"#0000FF");

// naks[i++] = new naksdata("హస్త"      ,"light"     ,"#00CCFF");

// naks[i++] = new naksdata("చిత్ర"      ,"soft"      ,"#11AC0D");

// naks[i++] = new naksdata("స్వాతి"      ,"mobile"    ,"#FF9900");

// naks[i++] = new naksdata("విశాక"    ,"mixed"     ,"#CC00FF");

// naks[i++] = new naksdata("అనురాధ"   ,"soft"      ,"#11AC0D");

// naks[i++] = new naksdata("జ్యేష్ట"    ,"sharp"     ,"#FF0000");

// naks[i++] = new naksdata("మూలా"       ,"sharp"     ,"#FF0000");

// naks[i++] = new naksdata("పూర్వాషాఢ"   ,"awful"     ,"#7D2000");

// naks[i++] = new naksdata("ఉత్తరాషాఢ"   ,"fixed"     ,"#0000FF");

// naks[i++] = new naksdata("శ్రవణ"    ,"mobile"    ,"#FF9900");

// naks[i++] = new naksdata("ధనిశ్త"   ,"mobile"    ,"#FF9900");

// naks[i++] = new naksdata("శతభిష"  ,"mobile"    ,"#FF9900");

// naks[i++] = new naksdata("పూర్వాభాద్ర"   ,"awful"     ,"#663300");

// naks[i++] = new naksdata("ఉత్తరాభాద్ర"   ,"fixed"     ,"#0000FF");

// naks[i++] = new naksdata("రేవతి"     ,"soft"      ,"#11AC0D");



naks[i++] = new naksdata("Ashwini", "light", "#00CCFF");

naks[i++] = new naksdata("Bharani", "awful", "#7D2000");

naks[i++] = new naksdata("Kritika", "mixed", "#CC00FF");
...
naks[i++] = new naksdata("Pushyami", "light", "#00CCFF");

naks[i++] = new naksdata("Ashelsha", "sharp", "#FF0000");

naks[i++] = new naksdata("Magha", "awful", "#7D2000");

naks[i++] = new naksdata("P. Phalguna", "awful", "#7D2000");

naks[i++] = new naksdata("U. Phalguna", "fixed", "#0000FF");

naks[i++] = new naksdata("Hasta", "light", "#00CCFF");

naks[i++] = new naksdata("Chitra", "soft", "#11AC0D");

naks[i++] = new naksdata("Swati", "mobile", "#FF9900");

naks[i++] = new naksdata("Vishaka", "mixed", "#CC00FF");

naks[i++] = new naksdata("Anuradha", "soft", "#11AC0D");

naks[i++] = new naksdata("Jyestha", "sharp", "#FF0000");

naks[i++] = new naksdata("Moola", "sharp", "#FF0000");

naks[i++] = new naksdata("P.Shadha", "awful", "#7D2000");

naks[i++] = new naksdata("U.Shadha", "fixed", "#0000FF");

naks[i++] = new naksdata("Shravana", "mobile", "#FF9900");

naks[i++] = new naksdata("Dhanistha", "mobile", "#FF9900");

naks[i++] = new naksdata("Shatabisha", "mobile", "#FF9900");

naks[i++] = new naksdata("P.Bhadra", "awful", "#663300");

naks[i++] = new naksdata("U.Badhra", "fixed", "#0000FF");

naks[i++] = new naksdata("Revathi", "soft", "#11AC0D");
...
function checkEntries(f) {
  ...
  var m = Math.floor(document.forms[0].Month.value);

  var y = Math.floor(document.forms[0].Year.value);

	var fields = ['Day', 'Month', 'Year', 'Hours', 'Minutes', 'Seconds'];
	var validInputs = [];
  for (i = 0; i < f.elements.length; i++) {
    var e = f.elements[i];
		if(fields.indexOf(e.name) !== -1) validInputs.push(e);;
	};
...

{
  ...
  document.forms[0].Month.value = d.getMonth();
...

{
  ...
  with(Math) {
    ...
    var day = floor(Number(document.forms[0].Day.value));

    var mon = floor(Number(document.forms[0].Month.value)) + 1;

    var year = floor(Number(document.forms[0].Year.value));

    var hr = floor(Number(document.forms[0].Hours.value));

    hr += floor(Number(document.forms[0].Minutes.value)) / 60;

    var tzone = abs(floor(Number(document.forms[0].ZoneHour.value)));

    tzone += floor(Number(document.forms[0].ZoneMin.value)) / 60;
  ...
  inpdate = day + ' ' + month[mon - 1] + ' ' + year + ' కోసం  <font color="#990000"><b>' + Math.floor(document.forms[0].Hours.value) + ':' + inpmin + '</b></font>'
...

{
  ...
  for (itit = n1; itit < (n1 + 2); ++itit) {
    ...
    if (itit == n1) s_t = calData(jdt + tzone / 24) + "&nbsp;&nbsp;కు ";
...
// Print panchanga from AstrologyTutorials.com
...
function results() {
  ...
  var date = a[0].split('/');
  var d1 = date[1] + '/' + date[0] + '/' + date[2];

  var date2 = a[1].split('/');
  var d2 = date2[1] + '/' + date2[0] + '/' + date2[2];

  // returns total mins difference
  nPadam = calculatePdm(d1, d2);

  var four = 0;
  four = parseInt(nPadam);

  var division = 0;
  //division = Math.ceil(four/4);
  division = parseInt(four / 4);


  var now = new Date(d1);
  var i = 0;
  var numOfOccurence = 4;
  var padams = new Array();
  var ActualPadam = '';
  var p = new Array();
  var hoursMinsArray = inpdate.split('<b>');
  hoursMins = hoursMinsArray[1].replace('</b></font>', '');
  var p1 = new Array();

  var day = Math.floor(Number(document.forms[0].Day.value));
  var mon = Math.floor(Number(document.forms[0].Month.value)) + 1;
  var year = Math.floor(Number(document.forms[0].Year.value));

  //p1[0]=new Date(""+curr_month + "/" + curr_date + "/" + curr_year +" " +curr_h + ":" + curr_m + ":" + curr_i+"");
  p1[0] = new Date(mon + '/' + day + '/' + year + ' ' + hoursMins);
  for (i = 1; i <= numOfOccurence; i++) {
    now.setMinutes(now.getMinutes() + division);
    var curr_date = now.getDate();
    var curr_month = now.getMonth() + 1; //Months are zero based
    var curr_year = now.getFullYear();
    var curr_h = now.getHours();
    var curr_m = now.getMinutes();
    var curr_i = now.getSeconds();
    p[i] = curr_date + "." + curr_month + "." + curr_year + " " + curr_h + ":" + curr_m;

    p1[i] = new Date("" + curr_month + "/" + curr_date + "/" + curr_year + " " + curr_h + ":" + curr_m + ":" + curr_i + "");
    var jdt2 = new Date(mon + '/' + day + '/' + year + ' ' + hoursMins);

    if (Date.parse(p1[i - 1]) <= Date.parse(jdt2) && Date.parse(jdt2) <= Date.parse(p1[i])) {
      ActualPadam = i;
    }
  }

  /**************************
   *Padam calculation end
   ***************************/




  panch = naks[n_naksh].nname;

  // document.all.vvv.innerHTML=panch;
  // swal('Your birth star: ' + panch, '', 'success');
  return panch;
...
function init() {
  ...
  var date = new Date();
  //var s = new DATE_FORMAT(NOW(),'%c.%d.%Y.%h:%i');
...
function calculatePdm(d1, d2) {
  ...
  if (isNaN(sec)) {
    ...
    swal('ERROR', "Input data is incorrect!", 'error');
    return;
  }
  if (sec < 0) {
    // alert("The second date ocurred earlier than the first one!");
    swal('ERROR', "The second date ocurred earlier than the first one!", 'error');
    return;
  }
  ...
  a[0] = trunc(sec / hour);
  a[1] = trunc(sec / minute);
  a[2] = trunc(sec / second);
